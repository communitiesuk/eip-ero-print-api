...
internal class S3ServiceTest {
    ...
    @Nested
    inner class PutObjectToTargetBucketFromByteArray {
        @Test
        fun `should put object data into a bucket`() {
            // Given
            val bucketName = s3Properties.vcaTargetBucket
            val key = "test-key"
            ...
            // When
            s3Service.putObjectToTargetBucketFromByteArray(key, data, false)

            // Then
            verify(s3Client).putObject(
                eq(PutObjectRequest.builder().key(key).bucket(bucketName).build()),
                argThat<RequestBody> { requestBody -> requestBodyMatchesByteArray(requestBody, data) }
            )
        }

        @Test
        fun `should put object data into a bucket and tagged as temporary object for deletion`() {
            // Given
            val bucketName = s3Properties.vcaTargetBucket
            val key = "test-key"
            val data = "test-data".toByteArray()
            ...
            // When
            s3Service.putObjectToTargetBucketFromByteArray(key, data, true)

            // Then
            verify(s3Client).putObject(
                eq(
                    PutObjectRequest.builder().key(key).bucket(bucketName)
